...
class chatting(Command):
    ...
    async def on_message(self, message):
        ...
        if message.content.startswith(prefix+"온도"):
            try:
                ...
                embed=discord.Embed(title="⚠ 오류", description="시스템에서 온도를 불러오는데에 실패했습니다.```py\n{error}\n```".format(error=error),color=0xff0909)
                await message.channel.send(embed=embed)
        ...
        if message.content.startswith(prefix+"거꾸로"):
            try:
                await message.delete()
            except:
                pass
        ...
        if message.content.startswith(prefix+"서버랭"):
            rank = {}
        ...
        if message.content.startswith(prefix+"업타임"):
            ...
            embed=discord.Embed(title=prefix+"업타임", description="봇이 동작한 시간은  %s시간 %s분 %s초 입니다." %(hours,minitues,seconds) , color=0x237ccd)

            await message.channel.send(embed=embed)


        if message.content.startswith(prefix+"도움") or message.content.startswith(prefix+'help'):
            a = message.content
            a = a[5:]
            if a == "":
                embed=discord.Embed(title="📜 도움말", description="봇의 사용을 도와줄 도움말입니다. 다음 명령어 그룹들을 참고하세요.", color=0x237ccd)
                embed.add_field(name=prefix+"도움 음악", value="음악 도움말입니다."+prefix+"도움 음악 또는 "+prefix+"help 를+ 참고하세요.", inline=False)
                embed.add_field(name=prefix+"도움 게임", value="봇에 있는 게임 기능에 관련된 도움말입니다. 자세한 명령어는 "+prefix+"도움 게임을 참고하세요.", inline=True)
                embed.add_field(name=prefix+"도움 기능", value="봇에 있는 기능에 대해 알려드립니다.", inline=True)
                embed.add_field(name=prefix+"도움 어드민", value="어드민이 서버 관리를 위해 사용 가능한 기능입니다. 자세한 명령어는 "+prefix+"도움 어드민을 참고하세요.", inline=True)
                ...
                embed=discord.Embed(title=" ", description="봇에 있는 게임 기능을 설명합니다.", color=0x237ccd)
                embed.add_field(name=prefix+"돈", value="자신의 돈을 확인합니다.", inline=False)
                embed.add_field(name=prefix+'랭킹 또는 '+prefix+'랭크', value='서버 전체의 돈 순위를 봅니다.')
                embed.set_footer(text="도움 명령어에 없는 명령어가 있을 수 있습니다.")
                try:
                    await message.author.send(embed=embed)
                    await message.channel.send("DM으로 메시지를 보냈습니다. 확인하세요.")
            ...
            #         await message.channel.send(embed=embed)
                ...
                embed.add_field(name=prefix+"프사 @상대", value="멘션한 상대의 프로필 사진을 가져옵니다. 상대를 지정하지 않으면 자신의 프로필 사진을 가져옵니다.", inline=False)
                embed.add_field(name=prefix+"백과사전 <검색어>", value="백과사전에서 검색어를 검색해줍니다.", inline=False)
                embed.add_field(name=prefix+"나무위키 <검색어>", value="해당 나무위키 검색어로 바로가는 나무위키 링크를 표시하고, 문서를 일부분 미리 볼 수 있습니다.")
                # embed.add_field(name=prefix+"도서검색 <검색어>", value="도서를 검색해줍니다.", inline=False)
                embed.add_field(name=prefix+"afk/잠수 <사유>", value="잠수를 선언합니다. 다시 돌아오면 환영해드립니다.", inline=False)
                embed.add_field(name=prefix+"자동번역 <번역할 문장>", value="언어를 자동으로 인식한 후 한국어로 번역합니다.")
                embed.add_field(name=prefix+"한글영어번역(영어한글번역, 일어한글번역, 한글일어번역) <번역할 문장>", value="선택한 언어에서 선택한 언어로 번역해줍니다.", inline=False)

                # embed.add_field(name=prefix+"죽창 <개수>", value="죽창을 표시합니다. 60개가 최대입니다.",inline=False)
                embed.add_field(name=prefix+"지진", value="지진 정보를 표시합니다.", inline=False)
                embed.add_field(name=prefix+"별명변경 <바꿀별명>", value="입력한 별명으로 별명을 변경합니다.", inline=False)
                embed.add_field(name=prefix+"조의 표해", value="봇이 조의를 표해줍니다.", inline=False)
                embed.add_field(name=prefix+"고양이/냥이", value="랜덤으로 고양이짤을 보여준다냐!", inline=False)
                embed.add_field(name=prefix+"강아지", value="랜덤으로 강아지짤을 보여준다멍.", inline=False)
                # embed.add_field(name=prefix+"원주율 구해", value="원주율을 1997자리 까지 구합니다.", inline=False)
                embed.add_field(name=prefix+"리마인더 <시간(초)> <사유(선택)>", value="선택한 초 있다가 알려드려요.", inline=False)
                ...
                # embed.add_field(name=prefix+"11번가 검색 <검색어>", value="11번가에서 검색해, 정보를 불러옵니다.", inline=False)
                ...
                embed.add_field(name="더 많은 기능은?", value="궁금증이나 도움 명령어에 수록되지 않은 명령어는 블맨#5969 으로 친추후 DM해주세요!", inline=False)

                # embed.add_field(name=prefix+"명언은?", value="명언을 표시합니다. (명언인지 확인안됨)", inline=False)
                # embed.add_field(name=prefix+"서버 인원은?", value="채팅한 서버의 인원을 표시합니다.", inline=False)
                ...
                embed=discord.Embed(title=" ", description="봇이 틀어주는 신나는 음악!", color=0x237ccd)
                embed.add_field(name='summon', value='현재 계시는 보이스채널에 들어갑니다. 음악 재생전 먼저 **꼭** 해주세요.')
                embed.add_field(name='play <유튜브주소>', value='음악을 재생합니다.', inline=False)
                embed.add_field(name='pause', value='음악을 잠시 멈춤니다.')
                embed.add_field(name='resume', value='음악을 다시 재생합니다.', inline=False)
                embed.add_field(name='volume <1~100>', value='음악의 소리를 조절합니다.')
                embed.add_field(name='stop', value='음악 플레이어를 초기화합니다.', inline=False)
                embed.add_field(name='search <유튜브검색어>', value='음악을 검색하고 고른 음악을 재생합니다.')
                embed.add_field(name='np <유튜브주소>', value='현재 재생중인 음악을 봅니다.', inline=False)
                embed.add_field(name='skip', value='음악을 스킵합니다.')
                embed.add_field(name='참고하세요', value='음악기능은 [Just-Some-Bots 클릭](https://github.com/SeulSi/MusicBot-1) 에서 보실 수 있습니다. 자세한 도움 명령어는 `'+prefix+'help` 를 참고하세요.', inline=False)
                try:
                    await message.author.send(embed=embed)
                except:
                    embed=discord.Embed(title="⚠ 주의", description="DM 보내기에 실패하였습니다. 계정에서 DM 설정을 확인해주세요.",color=0xd8ef56)
                ...
                embed.add_field(name=prefix+"킥 @유저", value="선택한 유저를 킥합니다.", inline=False)
                embed.add_field(name=prefix+"밴 @유저", value="선택한 유저를 밴합니다.", inline=False)
                embed.add_field(name=prefix+"언밴 @유저 또는 유저 ID ", value="선택한 유저를 언밴합니다. 유저 ID는 데스크톱 버전에서 오른쪽키 > ID복사로 얻으실 수 있습니다.", inline=False)
                embed.add_field(name=prefix+"뮤트 @유저", value="유저를 해당 채널에서 뮤트시킵니다.", inline=False)
                embed.add_field(name=prefix+"전체뮤트", value="명령어를 사용한 채널을 관리자 제외 모든 유저가 사용할 수 없도록 합니다.", inline=False)
                embed.add_field(name=prefix+"언뮤트 @유저", value="유저를 해당 채널에서 언뮤트시킵니다.", inline=False)
                embed.add_field(name=prefix+"전체언뮤트", value="전체뮤트를 해제합니다.", inline=False)
                embed.add_field(name=prefix+"커스텀 추가 <명령어>/<봇의 대답>", value="해당 서버만 사용되는 커스텀 명령어를 추가합니다. 명령어와 봇의 대답 구분에는 꼭 /가 필요합니다.", inline=False)
                embed.add_field(name=prefix+"커스텀 수정 <수정할 명령어>/<봇의 대답>", value="이미 추가된 커스텀 명령어를 수정합니다. 명령어와 봇의 대답 구분에는 꼭 /가 필요합니다.", inline=False)
                embed.add_field(name=prefix+"커스텀 보기", value="해당 서버의 모든 커스텀 명령어를 출력합니다.", inline=False)
                embed.add_field(name=prefix+"커스텀 삭제 [삭제할 커스텀 명령어]", value="해당 서버의 커스텀 명령어중 입력한 명령어를 삭제합니다.", inline=False)

                embed.add_field(name=prefix+"커스텀 초기화", value="해당 서버의 모든 커스텀 명령어를 삭제합니다.", inline=False)
                embed.add_field(name="더 많은 기능은?", value="궁금증이나 도움 명령어에 수록되지 않은 명령어는 블맨#5969으로 친추후 DM해주세요!", inline=False)
            ...
            #     embed.add_field(name=prefix+"철컹철컹", value="??? : 철컹", inline=False)
            #     try:
            #         await message.author.send(embed=embed)
            #     except:
            #         embed=discord.Embed(title="⚠ 주의", description="DM 보내기에 실패하였습니다. 계정에서 DM 설정을 확인해주세요.",color=0xd8ef56)
        ...
        if message.content.startswith(prefix+"안녕") or message.content.startswith(prefix+"안냥") or message.content.startswith(prefix+"ㅎㅇ") or message.content.startswith(prefix+"gd") or message.content.startswith(prefix+"hello"):
            ...
            embed = discord.Embed(title="👋 반가워요.", description="저는 아직 어린 신사에요!\n점점 발전하고 있으니 잘 봐주세요!\n\n기능추가문의: 블맨#5969" ,color=colorers)
            embed.set_thumbnail(url=bot_profile)
            await message.channel.send(embed=embed)


        if message.content.startswith(prefix+"별명변경"):
            try:
                a = message.content
                a = a[6:]
                b = a.lstrip()
        ...
        if message.content.startswith(prefix+"별명 초기화") or message.content.startswith(prefix+"별명초기화"):
            try:
                memberid = message.author.id
                member = message.guild.get_member(memberid)
        ...
        if message.content.startswith(prefix+"시간계산"):
            try:
                if not message.content[6:] == "":
                    answer = message.content[6:].lstrip()
                else:
                    embed=discord.Embed(title=prefix+"시간계산", description="yyyy-mm-dd 형식으로 입력해주세요.",color=0x237ccd)
                    await message.channel.send(embed=embed)
                    def usercheck(a):
        ...
        if message.content.startswith(prefix+"리마인더"):
            a = message.content[6:]
            a = a.lstrip()
            a = a.split()
            try:
                ...
                embed=discord.Embed(title="❌ 오류 발생", description=prefix+"리마인더 <시간(초)> <사유(선택)> 형식으로 사용해주세요. \n```%s```     "%(error) ,color=0xff0909)    
                await message.channel.send(embed=embed)


        if message.content.startswith(prefix+"히오스는?"): 
        ...
        if message.content.startswith(prefix+"기상특보"):
            async with aiohttp.ClientSession() as session:
        ...
        if message.content.startswith(prefix+"뽑기"):
...
#💨
        ...
        if message.content.startswith(prefix+"영어한글번역"):
        ...
        if message.content.startswith(prefix+"한글영어번역"):
            a = message.content[8:].lstrip()
        ...
        if message.content.startswith(prefix+"일어한글번역"):
            a = message.content[8:].lstrip()
        ...
        if message.content.startswith(prefix+"한글일어번역"):
            a = message.content[8:].lstrip()
        ...
        if message.content.startswith(prefix+"자동번역"):
            a = message.content[6:].lstrip()
        ...
        if message.content.startswith(prefix+"백과사전"):
            try:
                a = message.content[6:].lstrip()
                if a == "":
                    embed=discord.Embed(title="⚠ 주의", description="검색어가 없습니다.",color=0xd8ef56)
        ...
        if message.content.startswith(prefix+"링크축약") or message.content.startswith(prefix+"링크단축") or message.content.startswith(prefix+"주소단축") or message.content.startswith(prefix+"주소축약"):
            a = message.content[6:].lstrip()
        ...
        if message.content.startswith(prefix+"나무위키"):
            a = message.content
            a = a[7:]
            title = a
        ...
        if message.content.startswith(prefix+"서버리스트"):
        ...
        if message.content.startswith(prefix+"프레타는?"):
            send = ["??? : 그말 꺼내지 마세요.", "???", "@.@", "불-편", "안사요"]
            await message.channel.send(random.choice(send))

        if message.content.startswith(prefix+"냥이") or message.content.startswith(prefix+"고양이"):
            while True:
                async with aiohttp.ClientSession() as session:
        ...
        if message.content.startswith(prefix+"강아지") or message.content.startswith(prefix+"댕댕이"):
            async with aiohttp.ClientSession() as session:
        ...
        if message.content.startswith(prefix+"네이버 실검") or message.content.startswith(prefix+"네이버 실시간검색어") or message.content.startswith(prefix+"네이버 실시간 검색어") or message.content.startswith(prefix+"네이버실검"):
            async with aiohttp.ClientSession() as session:
        ...
        if message.content.startswith(prefix+"다음 실검") or message.content.startswith(prefix+"다음 실시간검색어") or message.content.startswith(prefix+"다음 실시간 검색어") or message.content.startswith(prefix+"다음실검"):
            async with aiohttp.ClientSession() as session:
        ...
        if message.content.startswith(prefix+"질문"):
        ...
        # if message.content.startswith(prefix+"재시작"):
        #     if message.author.id == 289729741387202560:

        #         try:
        #             embed=discord.Embed(title=prefix+"재시작", description="봇이 재시작 합니다.",color=0x237ccd )
        #             await message.channel.send(embed=embed)
        #             restart_bot()
        ...
        #             await message.channel.send(embed=embed)
        #     else:
        #         embed=discord.Embed(title="⚠ 주의", description=prefix+"오너만 사용 가능한 명령어입니다.",color=0xd8ef56)
        #         await message.channel.send(embed=embed)
